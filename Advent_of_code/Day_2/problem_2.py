...
def calculate_power_cube(dict_cube_min):
    power = 1
    for key in dict_cube_min.keys():
        power = power * dict_cube_min[key] 
    return power
...
def main():
    ...
    sum_ids = 0
    for line in lines:
        _, dict_cube_min = calculate_cube(line)
        power_cube = calculate_power_cube(dict_cube_min)
        sum_ids+=power_cube
    print(f"the result is {sum_ids}")
